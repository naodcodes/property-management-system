...
import { defaultLocale } from '@/i18n/config'; // Added defaultLocale
...
// Updated: Accepts locale for proper internationalization
...
export default function TenantDashboardPage() {
  ...
  const nextPaymentDue = nextInvoice?.due_date ? formatDate(nextInvoice.due_date, locale) : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {loading ? (
          Array.from({ length: 4 }).map((_, index) => (
            <div key={`skeleton-${index}`} className="rounded-[32px] bg-white p-8 shadow-sm">
          ...
          <>
            {/* Lease Status */}
            <div className="group relative bg-white p-8 rounded-[32px] border-2 border-transparent hover:border-stone-900 transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1 flex flex-col min-h-[260px]">
              ...
              {errors.lease ? (
                <>
                  ...
                  <p className="text-stone-400 text-xs font-medium">{t('unableToLoad')}</p>
                </>
              ) : lease ? (
                <>
                  ...
                  <p className="text-stone-400 text-xs font-medium">
                    {formatDate(lease.start_date, locale)} → {formatDate(lease.end_date, locale)}
                  </p>
                </>
              ) : (
                <>
                  <h3 className="text-5xl font-black text-stone-900 mb-2">—</h3>
                  <p className="text-stone-400 text-xs font-medium">{t('noActiveLease')}</p>
                </>
              )}
            ...
            <div className="group relative bg-white p-8 rounded-[32px] border-2 border-transparent hover:border-stone-900 transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1 flex flex-col min-h-[260px]">
              ...
              {errors.invoices ? (
                ...
                <>
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className={cn('text-3xl font-black', invoiceIsOverdue ? 'text-red-600' : 'text-stone-900')}>
              ...
              <div className="mt-auto pt-4">
                <Link href={`${localePrefix}/payments`} className="text-xs font-black text-amber-700 uppercase tracking-widest hover:text-amber-900">
                  {t('viewPayments')} →
                </Link>
              </div>
            </div>
            ...
            <div className="group relative bg-white p-8 rounded-[32px] border-2 border-transparent hover:border-stone-900 transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1 flex flex-col min-h-[260px]">
              ...
              {errors.maintenance ? (
                ...
                <>
                  <h3 className="text-3xl font-black text-stone-900 mb-2">{openTickets ?? 0}</h3>
                  <p className="text-stone-400 text-xs font-medium">
                    {openTickets && openTickets > 0 ? t('openRequests') : t('allResolved')}
                  </p>
                </>
              )}
              <div className="mt-auto pt-4">
                <Link href={`${localePrefix}/maintenance`} className="text-xs font-black text-amber-700 uppercase tracking-widest hover:text-amber-900">
                  {t('viewRequests')} →
                </Link>
              </div>
            </div>
      ...
      <div className="flex flex-col gap-3 md:flex-row">
        <Link href={`${localePrefix}/payments`} className="bg-stone-900 text-white px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-amber-700 transition-all shadow-sm active:scale-95 text-center">
          {t('payRent')}
        </Link>
        <Link href={`${localePrefix}/maintenance`} className="bg-white border-2 border-stone-900 text-stone-900 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-stone-900 hover:text-white transition-all shadow-sm active:scale-95 text-center">
          {t('reportIssue')}
        </Link>
        <Link href={`${localePrefix}/lease`} className="bg-white border-2 border-stone-900 text-stone-900 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-stone-900 hover:text-white transition-all shadow-sm active:scale-95 text-center">
          {t('viewLease')}
        </Link>
      </div>
    </div>
